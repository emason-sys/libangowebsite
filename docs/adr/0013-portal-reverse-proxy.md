...
Add a new dedicated subdomain `portal.libangolr.net` served by `dev-webserver` and reverse-proxy the entire backend UI through it. Namespaces are cleanly isolated because each Next.js app owns its own domain root. On top of the proxy, run a Cloudflare Turnstile challenge with a signed HMAC session cookie so bots can't harvest the proxied paths.

Concrete pieces:

- **DNS.** A record `portal.libangolr.net` → `132.145.24.253` (same OCI public IP as `devsite.libangolr.net`), authoritative at DNSmadeeasy.
- **TLS.** A separate Let's Encrypt certificate for `portal.libangolr.net`, obtained via `certbot certonly --nginx`, on the certbot-renew timer we already run.
...
- **CTAs.** The homepage and header links for "Retrieve Token" and "Become a Vendor" now point at `https://portal.libangolr.net/` and `https://portal.libangolr.net/vendor-applications/` respectively. The existing `/token` and `/apply` pages stay in the codebase and remain routable but are no longer linked from primary nav.
...
- **UX handoff.** Users see the URL change from `devsite.libangolr.net` to `portal.libangolr.net` when they click the CTA. This is standard portal-handoff UX (`portal.stripe.com`, `dashboard.aws`, etc.) and clearly signals "you are now in the app."
